...
def _parse_m_message(msg: str) -> Optional[tuple]:
    ...
    quote = {
        ...
        "prev_settle": _safe_float(parts[17]) if len(parts) > 17 else None,
...
class _MatbaWSClient:
    ...
    def _process_m(self, msg: str) -> None:
        ...
        if "DLR_SEP26" in security_id or "DLR_OCT26" in security_id:
            logger.info("RAW %s fields=%d msg=%s", security_id, len(msg.split("|")), msg)
        with self._lock:
