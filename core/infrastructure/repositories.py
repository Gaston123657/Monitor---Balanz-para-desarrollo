...
class ExcelInstrumentsRepository(IInstrumentsRepository):
    ...
    def _load_all_impl(self):
        try:
            ...
            for sheet in sheet_names:
                try:
                    ...
                    for _, row in df.iterrows():
                        ...
                        # ONs: legislacion (NY/AR) + sector — metadata estructural
                        # consumida por server._refresh_bond_panels para split de panel.
                        leg_raw = row.get("legislacion")
                        legislacion = str(leg_raw).strip().upper() if leg_raw is not None and not pd.isna(leg_raw) else None
            ...
            # Cerrar explícitamente: pd.ExcelFile sin close() deja el FD vivo
            # hasta que el GC pase, y en Windows eso bloquea `os.replace`
            # durante un save atómico (ABM, ons_sync, startup_purge) hasta que
            # el GC corra — timing no determinista bajo threading.
            xl.close()
        ...
        def _fetch_one(item):
            ...
            try:
                ...
                for row in payload:
                    ...
                    if not sym:
                        continue
                    # ONs (arg_corp) en pesos (sufijo O): la API publica precio
                    # ×1000 — los ARSO/MGCHO/etc vienen como 100000.0 en vez de
                    # 100. Normalizamos in-place para que sea transparente al
                    # consumidor. MEP (D) y Cable (C) NO llevan ese factor.
                    if name == "corp" and sym.endswith("O"):
                        for k in ("c", "px_bid", "px_ask"):
                            v = row.get(k)
