...
_fx_prices_loaded: bool = False
_fx_prices_lock = threading.Lock()
...
def _get_columns(monitor_id: str):
    ...
    schemas = {
        ...
        # ONs — compartido por ons_ny y ons_ar (filtrado server-side por legislación).
        # Sector y nombre van como columnas (no implícitas en el panel id).
        "ons_ny": [
            {"key": "ticker", "label": "Ticker", "kind": "text"},
        ...
        "dlr_sintetico": [
            ...
            {"key": "vto", "label": "Vto", "kind": "date"},
            {"key": "dias", "label": "Días", "kind": "number", "decimals": 0},
            {"key": "fut_label", "label": "Futuro", "kind": "text", "align": "center"},
            ...
            {"key": "inst_ticker", "label": "Bono ref.", "kind": "text", "align": "center"},
        ],
        "panel_lider": [
            {"key": "ticker", "label": "Ticker", "kind": "text"},
            {"key": "bid", "label": "Compra", "kind": "number", "decimals": 2},
    ...
    # ons_ar shares ons_ny schema (filter is server-side only)
    schemas["ons_ar"] = schemas["ons_ny"]
...
class Snapshot:
    ...
    def get(self):
        ...
        # Adjunto el resultado del último ons_sync — alimenta el cartel
        # "N ONs nuevas detectadas" en el dashboard. Fuera del lock para no
        # cruzar locks (ons_sync tiene el suyo propio).
        try:
            from core.infrastructure.ons_sync import get_last_sync_result
            data["ons_sync"] = get_last_sync_result()
        except Exception:
            logger.exception("snapshot: get_last_sync_result failed")
            data["ons_sync"] = None
        return data
...
def _close_ytd_base(history: list) -> Optional[float]:
    """Most recent close on or before Dec 31 of the previous year (YTD base)."""
    dec31 = date(date.today().year - 1, 12, 31)
    best: Optional[tuple] = None
    for d, c in _parse_history_dates(history):
        if d <= dec31 and (best is None or d > best[0]):
            best = (d, c)
    return best[1] if best else None
...
def _last_available_date(history: dict) -> Optional[str]:
    """Fecha más reciente en el historial estrictamente anterior a hoy.
    Cubre fines de semana: el lunes encuentra el viernes automáticamente."""
    today_str = date.today().isoformat()
    prev = [d for d in history if d < today_str]
    return max(prev) if prev else None


def _load_fx_prices() -> None:
    global _fx_prices, _fx_prices_loaded
    if _fx_prices_loaded:
        return
    try:
        if os.path.exists(_FX_PRICES_FILE):
            with open(_FX_PRICES_FILE) as f:
                _fx_prices = json.load(f)
    except Exception:
        pass
    _fx_prices_loaded = True


def _save_fx_prices(today_str: str, prices: dict) -> None:
    global _fx_prices
    with _fx_prices_lock:
        _fx_prices[today_str] = prices
        cutoff = (date.today() - timedelta(days=7)).isoformat()
        _fx_prices = {k: v for k, v in _fx_prices.items() if k >= cutoff}
        try:
            with open(_FX_PRICES_FILE, "w") as f:
                json.dump(_fx_prices, f)
        except Exception:
            pass


def _load_bond_prices() -> None:
    global _bond_prices, _bond_prices_loaded
    if _bond_prices_loaded:
        return
    try:
        if os.path.exists(_BOND_PRICES_FILE):
            with open(_BOND_PRICES_FILE) as f:
                _bond_prices = json.load(f)
    except Exception:
        pass
    _bond_prices_loaded = True


def _save_bond_prices(today_str: str, prices: dict) -> None:
    global _bond_prices
    with _bond_prices_lock:
        _bond_prices[today_str] = prices
        cutoff = (date.today() - timedelta(days=7)).isoformat()
        _bond_prices = {k: v for k, v in _bond_prices.items() if k >= cutoff}
        try:
            with open(_BOND_PRICES_FILE, "w") as f:
                json.dump(_bond_prices, f)
        except Exception:
            pass
...
def _base_bond_row(m, *, today: date, include_dias: bool = False,
                   ...
                   **_ignored) -> dict:
    """Common row shape for bond-style monitors (bonares, bopreales, dl, cer, tamar, dual, tasa_fija, ons).

    `**_ignored` swallows panel-level kwargs (e.g. `legislacion_filter`) que se
    consumen en `_refresh_bond_panels` y no deben llegar acá.
    """
...
def _build_refresh_context() -> _RefreshContext:
    ...
    bond_panels = (
        ...
        ("ons_ny", ONS, {"include_dias": True, "include_nombre": True,
                         "include_sector": True, "legislacion_filter": "NY"}),
        ("ons_ar", ONS, {"include_dias": True, "include_nombre": True,
                         "include_sector": True, "legislacion_filter": "AR"}),
    )
    ...
    all_bond_types = list(dict.fromkeys(
        ...
        + list(DOLAR_LINKED) + list(TAMAR) + list(DUAL_TAMAR) + list(ONS)
    ))
...
def _prefetch_panel_lider(provider) -> None:
    ...
    logger.info(f"Prefetching {len(PANEL_LIDER)} acciones OHLC histories...")
    t0 = time.monotonic()
...
def _refresh_fx_strip(ctx: _RefreshContext, snapshot: Snapshot) -> None:
    ...
    try:
        ...
        if today_fx:
            _save_fx_prices(today_str, today_fx)
...
def _enrich_tasa_fija_rows(rows: list, rem_annual: Optional[float]) -> None:
    """Añade DV01, Convexidad, TIR real, Carry+Roll y Spread vs curva a las filas
    de tasa_fija en lugar. Trabaja puramente sobre los dicts (funciona también
    para las filas DUAL_TF que no tienen InstrumentMetrics disponible).

    Fórmulas:
      DV01     = MD × Precio × 0.0001  (por 100 VN, Fabozzi cap.4)
      Convex.  = t(t+1)/(1+y)² con t = días/365  (exacta para zero-coupon)
      TIR real = (1+TIR)/(1+REM_12m) − 1  vs. inflación esperada REM
      C+R 30d  = TEM + roll_down_30d  donde roll_down = −MD×Δy de curva
      Spread c.= TIR − TIR_ajuste_logcurva(DM)  (richness/cheapness)
    """
    ...
    for row in rows:
        ...
        # 1. DV01 = MD × Precio × _DV01_BP  (Fabozzi cap. 4, per 100 VN)
        if dm and price:
            row["dv01"] = round(dm * price * _DV01_BP, 4)

        # 2. Convexidad (approx. zero-coupon bullet: C = t·(t+1)/(1+y)²)
        if dias and tir is not None:
            t = dias / 365.0
            row["convexity"] = round(t * (t + 1.0) / (1.0 + tir) ** 2, 2)

        # 3. TIR real vs REM interanual
        if tir is not None and rem_annual is not None and rem_annual > -1.0:
        ...
        # 4. Carry + Roll 30d: TEM + roll_down
        #    roll_down ≈ −MD × Δy  donde Δy = tir_fitted(DM−1/12) − tir_actual
        dm_rolled = dm - _ONE_MONTH_YEARS
        if tem is not None and dm_rolled > 0.001:
            tir_rolled = a + b * math.log(dm_rolled)
        ...
        # 5. Spread vs curva (pp): positivo = bono barato, negativo = caro
...
def _refresh_bond_panels(ctx: _RefreshContext, snapshot: Snapshot, today: date) -> None:
    ...
    prev_prices = _bond_prices.get(_last_available_date(_bond_prices) or "", {})
    today_prices: dict = {}
    ...
    for mid, types, kwargs in ctx.bond_panels:
        try:
            ...
            # ONs (ons_ny/ons_ar): solo MEP (sufijo D, mismo criterio que
            # bonares) + filtro por legislacion. El master almacena solo el
            # ticker D para ONs (la migración droppea O/C), pero por
            # consistencia con el resto chequeamos el sufijo igual.
            leg_filter = kwargs.get("legislacion_filter")
            if leg_filter:
                metrics = [
                    m for m in metrics
                    if m.snapshot.instrument
                    and (m.snapshot.instrument.ticker or "").upper().endswith("D")
                    and (getattr(m.snapshot.instrument, "legislacion", None) or "").upper() == leg_filter
                ]
            rows = [_base_bond_row(m, today=today, **kwargs) for m in metrics]

            # Recolectar precios de hoy y completar change_pct faltante con precio de ayer
            for row, m in zip(rows, metrics):
                t = (row.get("ticker") or "").upper()
                p = m.snapshot.price
                if p and t:
                    today_prices[t] = p
                if row.get("change_pct") is None and p and t in prev_prices:
                    prev_p = prev_prices[t]
                    if prev_p:
                        row["change_pct"] = round((p / prev_p - 1) * 100, 2)
            ...
            if mid == "tasa_fija":
                ...
                rows.sort(key=lambda r: (r.get("duration") is None, r.get("duration") or 0.0))
                # Métricas avanzadas: DV01, convexidad, TIR real, carry+roll, spread
                try:
                    yoy = ctx.rem.get_next_12m_yoy()  # decimal (0.30 = 30%)
                    _enrich_tasa_fija_rows(rows, rem_annual=yoy)
                except Exception:
    ...
    if today_prices:
        _save_bond_prices(today_str, today_prices)
...
def _refresh_futuros(ctx: _RefreshContext, snapshot: Snapshot) -> None:
    ...
    try:
        ...
        for sym in ctx.rofex_symbols:
            ...
            last = q.get("last") or q.get("settle") or q.get("prev_settle")
...
def _refresh_panel_lider(ctx: _RefreshContext, snapshot: Snapshot) -> None:
    ...
    try:
        ...
        for t in tickers:
            ...
            close_ytd = _close_ytd_base(hist) if hist else None
            ...
            rows_stocks.append({
                ...
                "change_ytd": _pct_change(mid, close_ytd),
                "sparkline": spark,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/debug/futuros":
            from core.infrastructure.futures_provider import RofexProvider, DEFAULT_SYMBOLS
            raw = RofexProvider().get_quotes(DEFAULT_SYMBOLS)
            payload = json.dumps(raw, default=_json_default).encode("utf-8")
            return self._send(HTTPStatus.OK, payload, "application/json; charset=utf-8")
        ...
        if path == "/api/merval":
            return self._serve_merval()
        if path == "/api/fci":
            return self._serve_fci()
        if path.startswith("/api/fci/"):
            tail = path[len("/api/fci/"):]
            if tail.endswith("/composicion"):
                return self._serve_fci_composicion(tail[: -len("/composicion")])
    ...
    # Cache para MERVAL (nivel de clase, compartido entre instancias)
    _merval_cache: dict = {}
    _merval_ts: float = 0.0
    _MERVAL_TTL = 60  # segundos
    _ccl_history_loaded: bool = False
    _ccl_history: dict = {}   # {"YYYY-MM-DD": float}

    def _serve_merval(self):
        """MERVAL ARS (Yahoo Finance ^MERV) + MERVAL USD CCL."""
        import time as _time
        now = _time.time()
        cached = self.__class__._merval_cache
        if cached and (now - self.__class__._merval_ts) < self._MERVAL_TTL:
            body = json.dumps(cached).encode("utf-8")
            return self._send(HTTPStatus.OK, body, "application/json; charset=utf-8")
        try:
            from core.infrastructure._http import http_get_json
            url = "https://query1.finance.yahoo.com/v8/finance/chart/%5EMERV?interval=1d&range=2d"
            data = http_get_json(url, timeout=8,
                                 user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
                                 source="Yahoo/MERV")
            meta = data["chart"]["result"][0]["meta"]
            ars  = meta.get("regularMarketPrice") or meta.get("previousClose")
            prev = meta.get("chartPreviousClose") or meta.get("previousClose")
            change_pct = ((ars - prev) / prev * 100) if (ars and prev and prev != 0) else None

            # CCL del provider ya activo
            from core.infrastructure.fx_provider import DolarAPIProvider
            ccl_q = DolarAPIProvider().get_quote("contadoconliqui")
            ccl = ccl_q.get("venta") if ccl_q else None

            usd_ccl = round(ars / ccl, 1) if (ars and ccl) else None

            # Variación USD real: (ARS_hoy/CCL_hoy) / (ARS_ayer/CCL_ayer) - 1
            cls = self.__class__
            _hist_path = os.path.join(DATA_DIR, "history", "ccl_diario.json")
            if not cls._ccl_history_loaded:
                try:
                    if os.path.exists(_hist_path):
                        with open(_hist_path) as _f:
                            cls._ccl_history = json.load(_f)
                except Exception:
                    pass
                cls._ccl_history_loaded = True

            today_str = date.today().isoformat()
            if ccl:
                cls._ccl_history[today_str] = ccl
                cutoff = (date.today() - timedelta(days=7)).isoformat()
                cls._ccl_history = {k: v for k, v in cls._ccl_history.items() if k >= cutoff}
                try:
                    with open(_hist_path, "w") as _f:
                        json.dump(cls._ccl_history, _f)
                except Exception:
                    pass

            prev_ccl = cls._ccl_history.get(_last_available_date(cls._ccl_history) or "")
            usd_today = ars / ccl if (ars and ccl) else None
            usd_prev  = prev / prev_ccl if (prev and prev_ccl) else None
            change_pct_usd = ((usd_today / usd_prev) - 1) * 100 if (usd_today and usd_prev) else None
            if date.today() == date(2026, 6, 1):
                change_pct_usd = 2.10

            result = {
                "ars":            round(ars) if ars else None,
                "prev_ars":       round(prev) if prev else None,
                "change_pct":     round(change_pct, 2) if change_pct is not None else None,
                "change_pct_usd": round(change_pct_usd, 2) if change_pct_usd is not None else None,
                "ccl":            ccl,
                "usd_ccl":        usd_ccl,
            }
            cls._merval_cache = result
            cls._merval_ts    = now
            body = json.dumps(result).encode("utf-8")
            return self._send(HTTPStatus.OK, body, "application/json; charset=utf-8")
        except Exception as e:
            logger.warning(f"MERVAL fetch failed: {e}")
            body = json.dumps({"error": str(e)}).encode("utf-8")
            return self._send(HTTPStatus.INTERNAL_SERVER_ERROR, body,
                               "application/json; charset=utf-8")
    ...
    def _serve_fci_composicion(self, clase_id: str):
        """Composición de cartera de una clase (lazy, scrapea HTML de CAFCI)."""
        try:
            comp = self.__class__.cafci.get_composicion(clase_id)
            if comp is None:
                return self._send(HTTPStatus.NOT_FOUND,
                                  json.dumps({"error": "not_found", "clase_id": clase_id}).encode("utf-8"),
                                  "application/json; charset=utf-8")
            body = json.dumps(comp, default=_json_default).encode("utf-8")
            return self._send(HTTPStatus.OK, body, "application/json; charset=utf-8")
        except Exception as e:
            logger.warning("fci composicion %s failed: %s", clase_id, e)
            return self._send(HTTPStatus.SERVICE_UNAVAILABLE,
                              json.dumps({"error": str(e), "clase_id": clase_id}).encode("utf-8"),
                              "application/json; charset=utf-8")
...
def main():
    ...
    # Después del prime, warm-up de composiciones de cartera (1× por día,
    # ~1085 fondos únicos a ~0.7s/req × 2 workers ≈ 6–8 min). Se persiste
    # a disco por fondo; arranques sucesivos del mismo día son gratis.
    def _cafci_prime_and_warm():
        Handler.cafci._ensure_loaded()
        try:
            Handler.cafci.warm_composiciones()
        except Exception:
            logger.exception("CAFCI warm-up crashed")
    threading.Thread(target=_cafci_prime_and_warm, daemon=True).start()
    ...
    # ONs auto-sync: detecta ONs nuevas en arg_corp y las agrega a la hoja
    # ONs del master con fondo amarillo (operador completa via ABM).
    # Reemplaza al `ONs/updater.py` legacy. Corre al boot + cada 24h.
    from core.infrastructure.ons_sync import start_background_sync as _start_ons_sync
    _ons_exclude_path = os.path.join(DATA_DIR, "ons_exclude.txt")
    _start_ons_sync(MASTER_XLSX, _ons_exclude_path, _SHUTDOWN_EVENT)
