...
│
├── data/
│   ├── instruments_master.xlsx         # FUENTE DE VERDAD: 6 hojas (Soberanos, Tasa_Fija, CER, Dolar_Linked, TAMAR, ONs)
│   │                                   #                   + Cashflows + Cashflows_Fija
│   ├── ons_exclude.txt                 # Bases de ticker que ons_sync.py ignora permanentemente
│   └── history/
...
| **ONs** | ticker, short_name, tipo, sector, legislacion, isin, fecha_emision, fecha_vencimiento, cupon anual %, frecuencia pagos, base calculo, tipo amortizacion, amort inicio, amort cantidad, amort frec, amort %, amort cant1, amort %2 | Obligaciones Negociables corporativas. `tipo = "ON"` siempre. `legislacion` ∈ {NY, AR} → split paneles ons_ny/ons_ar. Ticker = sufijo D (MEP). Cashflows synth-only (`_synth_on_bond`); soporta amort 2-tramos (cant1/%2) y amort frec ≠ cupón. ONs nuevas se autodetectan vía `core/infrastructure/ons_sync.py` (1×/día). |
